...
public class CameraManager{
    ...
    public static void getInsance(Context context){
...
//            int height = screenResolution.y * 4 / 5;
    ...
    public void openDriver(SurfaceHolder holder) throws IOException{
        if(camera ==null){
            Log.i(TAG,"open camera,打开相机");
            camera=Camera.open();
            if(camera == null){
                throw new IOException();
            }
            camera.setPreviewDisplay(holder);//设置surface用于实时预览。
...
//            camera.setDisplayOrientation(90);//旋转90度
            ...
            configManager.setDesiredCameraParameters(camera);//配置摄像头的各种参数
