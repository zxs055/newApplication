...
public class ProgressWheel extends View {
     ...
     *
     * @param context
     * @param attrs 代表xml里一个节点下面的属性的集合
     */
...
//        context.obtainStyledAttributes()方法返回得到属性的容器TypedArray
    }
     ...
     * 从XML中解析控件的属性
     *
     * @param a the attributes to parse存储资源数组的容器
     */
    private void parseAttributes(TypedArray a) {
        barWidth = (int) a.getDimension(R.styleable.ProgressWheel_mProg_barWidth,
                barWidth);

        rimWidth = (int) a.getDimension(R.styleable.ProgressWheel_mProg_rimWidth,
                rimWidth);

        spinSpeed = (int) a.getDimension(R.styleable.ProgressWheel_mProg_spinSpeed,
                spinSpeed);

        delayMillis = a.getInteger(R.styleable.ProgressWheel_mProg_delayMillis,
                delayMillis);
        if (delayMillis < 0) {
            delayMillis = 0;
        }

        barColor = a.getColor(R.styleable.ProgressWheel_mProg_barColor, barColor);

        barLength = (int) a.getDimension(R.styleable.ProgressWheel_mProg_barLength,
                barLength);

        textSize = (int) a.getDimension(R.styleable.ProgressWheel_mProg_textSize,
                textSize);

        textColor = (int) a.getColor(R.styleable.ProgressWheel_mProg_textColor,
                textColor);

        //如果text是空的，就无视它
        if (a.hasValue(R.styleable.ProgressWheel_mProg_text)) {
            setText(a.getString(R.styleable.ProgressWheel_mProg_text));
        }

        rimColor = (int) a.getColor(R.styleable.ProgressWheel_mProg_rimColor,
                rimColor);

        circleColor = (int) a.getColor(R.styleable.ProgressWheel_mProg_circleColor,
                circleColor);

        contourColor = a.getColor(R.styleable.ProgressWheel_mProg_contourColor, contourColor);
        contourSize = a.getDimension(R.styleable.ProgressWheel_mProg_contourSize, contourSize);
